...
if (serverCount.count === 0) {
  ...
  const servers = [
    ...
    },
    {
      ...
      publicKey: "5+g23xQ4tw/BYu+fyJeIFP4vdMtBDxdDI1cD1ZJ+YFM=",
      isPremium: 0,
      isActive: 0,
      comingSoon: 1,
    },
...
class DatabaseService {
  getUserKeys(userId) {
    const row = db
      .prepare("SELECT * FROM user_keys WHERE userId = ?")
      .get(userId);
    return row
  ...
  deleteUser(userId) {
    ...
    // Дублируем в PostgreSQL
    syncToPg(async () => {
      await pgClient.query('DELETE FROM vpn_user_keys WHERE "userId" = $1', [
        userId,
      ]);
