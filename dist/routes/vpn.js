...
router.get("/devices", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await pgClient.query('SELECT * FROM vpn_devices WHERE "userId" = $1 ORDER BY "createdAt" ASC', [userId]);
...
router.post("/devices", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const count = await pgClient.query('SELECT COUNT(*) FROM vpn_devices WHERE "userId" = $1', [userId]);
        ...
        const publicKey = (0, child_process_1.execSync)(`echo "${privateKey}" | wg pubkey`)
            .toString()
            .trim();
        // Генерируем IP
    ...
    `, [
        ...
        (0, child_process_1.execSync)(`wg set wg0 peer ${publicKey} allowed-ips ${ip}/32`);
...
router.get("/devices/:id/qr", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await pgClient.query('SELECT * FROM vpn_devices WHERE id = $1 AND "userId" = $2', [deviceId, userId]);
...
router.post("/devices/:id/subscribe", auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user.userId;
        const deviceId = req.params.id;
        // Получаем устройство
        const deviceRes = await pgClient.query('SELECT * FROM vpn_devices WHERE id = $1 AND "userId" = $2', [deviceId, userId]);
        ...
        // Проверяем баланс
        const balanceRes = await pgClient.query('SELECT amount FROM balances WHERE "userId" = $1', [userId]);
        ...
        // Списываем баланс
        await pgClient.query('UPDATE balances SET amount = amount - $1 WHERE "userId" = $2', [price, userId]);
        // Активируем подписку
        const months = billingType === "yearly" ? 12 : 1;
        const subscriptionEndsAt = new Date(Date.now() + months * 30 * 24 * 60 * 60 * 1000);
        await pgClient.query(`
  UPDATE vpn_devices 
...
`, [plan, billingType, subscriptionEndsAt, newIp, deviceId, userId]);
        ...
        try {
            const wgPeers = (0, child_process_1.execSync)("wg show wg0 peers").toString();
            if (!wgPeers.includes(device.publicKey)) {
                (0, child_process_1.execSync)(`wg set wg0 peer ${device.publicKey} allowed-ips ${newIp}/32`);
                (0, child_process_1.execSync)("wg-quick save wg0");
    ...
    try {
        ...
        if (plan === "standard" || plan === "basic") {
            ...
            if (currentIpNum < 128) {
                ...
                try {
                    (0, child_process_1.execSync)(`wg set wg0 peer ${device.publicKey} remove`);
                    (0, child_process_1.execSync)(`wg set wg0 peer ${device.publicKey} allowed-ips ${newIp}/32`);
                    (0, child_process_1.execSync)("wg-quick save wg0");
...
router.get("/admin/stats", auth_1.authMiddleware, async (req, res) => {
    try {
      ...
      FROM vpn_devices
    `);
        const usersStats = await pgClient.query("SELECT COUNT(*) as total FROM users");
        // CPU и RAM
        const cpuLoad = (0, child_process_1.execSync)("top -bn1 | grep 'Cpu(s)' | awk '{print $2}'")
            .toString()
            .trim();
